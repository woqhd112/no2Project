	...
	<div class="wow fadeInUp">
		<div class="lms">
			<div class="conHeader">
				...
				<div id="headerBtn">
					<a href="${root }LMS/Account/inforup.html?empnum=<%=login.getEmpnum()%>">정보수정</a>
					<a href="${root }LMS/logout.html">로그아웃</a>
				</div>
			</div>
			<div class="box">
				<ul class="stuMenus">
					<li id="check"><a style="cursor: default;" href="${root }LMS/Account/accountTeam.html">◎&nbsp;&nbsp;교직원 정보</a></li>
				</ul>
				<ul class="mainbox">
					...
					<li>
						<div id="info">
							...
							<% Paging paging = (Paging)request.getAttribute("paging");
								...
								if(prev==true){
							%>
								<a class="prnx" href="${root }LMS/Account/accountTeam.html?page=<%=begin-5 %>&search=<%=search%>&keyword=<%=keyword%>">＜</a>
							<%	}
							...
							<%		}else{ %>
								<a class="page" href="${root }LMS/Account/accountTeam.html?page=<%=i %>&search=<%=search%>&keyword=<%=keyword%>">&nbsp;<%=i %>&nbsp;</a>
							<%		}}
								if(next==true){	
							%>
								<a class="prnx" href="${root }LMS/Account/accountTeam.html?page=<%=end+1 %>&search=<%=search%>&keyword=<%=keyword%>">＞</a>
							<%} 
							%>
							</div>
							<div id="search">
								<form action="${root }LMS/Account/accountTeam.html">
									...
									<input style="border-radius: 3px; height: 32px; font-size:12px;" class="searchbtn" type="submit" value="검색">
			              			<input style="border-radius: 3px; height: 32px; font-size:12px;" class="resetbtn" type="button" onclick="location.href='./accountTeam.html'" value="초기화">
								</form>
							</div>
							<br/>
							<br/>
