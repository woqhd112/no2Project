			...
			<ul class="mainbox">
				...
				<li>
					<div id="info">
						<table>
							...
							<tbody>
								<c:forEach items="${alist }" var="bean">
									<tr>
										<td><a href="${root }LMS/Govern/studentAdmin_detail.html?num=${bean[0].stunum}">${bean[0].stuname }</a></td>
										<td>${bean[0].classname }</td>
										<c:choose>
											<c:when test="${bean[1].arrtime eq null }">
												<td style="font-size: 14px; font-weight: bold; color:darkred">X</td>
											</c:when>
											<c:otherwise>
												<td style="font-size: 14px; font-weight: bold;">O</td>
											</c:otherwise>
										</c:choose>
										<td>${bean[2].checkpct }%</td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
						...
						<%		}else{ %>
							<a class="page" href="${root }LMS/Govern/studentAdmin.html?page=<%=i %>&search=<%=search%>&keyword=<%=keyword%>">&nbsp;<%=i %>&nbsp;</a>
						<%		}}
							if(next==true){	
						%>
							<a class="prnx" href="${root }LMS/Govern/studentAdmin.html?page=<%=end+1 %>&search=<%=search%>&keyword=<%=keyword%>">＞</a>
						<%} 
						%>
					</div>
						<div id="search">
