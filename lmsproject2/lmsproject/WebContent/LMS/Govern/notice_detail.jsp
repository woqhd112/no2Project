...
LMS_Notice_Dto bean = (LMS_Notice_Dto)request.getAttribute("bean");
%>
	$(document).ready(function(){
		...
		$('.btn1_1').on("click",function(){
			...
			if(modify){
				$.post('${root}LMS/Govern/notice_detail.html','nottitle='+$('#newTitle').val()+'&notcontent='+$('#textarea2').val()+'&notnum=<%=bean.getNotnum() %>',function(data){
				if(data=='ok'){
	            	alert('수정이 완료되었습니다');
		...
		//삭제
      	$('.btn2').on("click",function(){
			...
			if(del){
			$.post('${root}LMS/Govern/notice_delete.html','notnum=<%=bean.getNotnum() %>',function(data){
			if(data=='ok'){
				alert('삭제가 완료되었습니다.');
				window.location.href='${root}LMS/Govern/notice.html';
			}else if(data=='fail'){
				alert('삭제실패');
			}else if(data=='fail1'){
				alert('데이터가 비정상적으로 접근하였습니다.');
			...
			<ul class="mainbox">
				...
				<li>
						...
						<table>
							<thead>
								<tr>
									...
									<td id="title_1" class="title" colspan="3">
										<%=bean.getNottitle() %>
									</td>
									<td id="title_2" colspan="3">
										<input id="newTitle" type="text" value="<%=bean.getNottitle() %>" style="height: 30px; padding-left:5px; width: 97%"/>
									</td>
								</tr>
								<tr>
									<th style="width:15%">작성일</th>
									<td style="width:35%"><%=bean.getNotdate() %></td>
							...
							<tbody>
								<tr>
									<td id="content_1" colspan="4" >
										<textarea id="textarea1" class="textarea" style="width:98%;" readonly="readonly"><%=bean.getNotcontent() %></textarea>
									</td>
									<td id="content_2" colspan="4">
										<textarea id="textarea2" class="textarea" style="width: 98%; border:0.5px solid silver; border-top: none;"><%=bean.getNotcontent() %></textarea>
									</td>
								</tr>
							</tbody>
						</table>
					...
					<div class="btnbox">
						...
						<button class="btn4" type="button" onclick="location.href='notice.html'">리스트</button>
					</div>
				</li>
			</ul>
		</div>
