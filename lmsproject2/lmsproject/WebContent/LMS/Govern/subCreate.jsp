...
<body>
	<div class="lms">
		...
		<div class="box">
			...
			<ul class="mainbox">
				<li class="title" style="margin-top:50px; margin-bottom:10px">강좌 개설</li>	
				<li>
					<input style=" margin-bottom: 15px;" class="addbtn" type="button" onclick="location.href='${root}LMS/Govern/subCreate_insert.html'" value="등록">
				</li>
				<li>
					<div id="regnot">
						<c:forEach items="${alist }" var="bean">
							<div id="regnottitle">
								<a href="${root }LMS/Govern/subCreate_detail.html?num=${bean.classnum}">${bean.teachername } 강사 | ${bean.classname }</a>
							</div>
						</c:forEach>
						...
						<%
							...
							if(prev==true){
						%>
							<a class="prnx" href="${root }LMS/Govern/subCreate.html?page=<%=begin-10 %>">＜</a>
						<%	}
						...
						<%		}else{ %>
							<a class="page" href="${root }LMS/Govern/subCreate.html?page=<%=i %>">&nbsp;<%=i %>&nbsp;</a>
						<%		}}
							if(next==true){	
						%>
							<a class="prnx" href="${root }LMS/Govern/subCreate.html?page=<%=end+1 %>">＞</a>
						<%} 
						%>
					</div>
					</div>
