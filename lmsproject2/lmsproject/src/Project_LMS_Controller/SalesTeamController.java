...
@WebServlet("/LMS/Sales/salesTeam.html")
public class SalesTeamController extends HttpServlet{
		...
		//검색
			...
			String tablename = "emp1";
				try {
					...
					System.out.println("영업팀 교직원정보 화면출력-테이블갯수 에러");
			...
			try{
				...
				System.out.println("영업팀 교직원정보 화면출력 에러");
				resp.sendRedirect("../lmsLogin.html");
				req.getSession().setAttribute("lmslogin", null);
				return;
			
			}
		req.getRequestDispatcher("salesTeam.jsp").forward(req, resp);
	}
}
